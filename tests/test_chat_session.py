...
def test_prepare_request():
    ...
    model, other_options, history, user_message, _ = ai.prepare_request(
...
def test_prepare_model_request():
    ...
    llm_options: LLMOptions = {"model": "test-model", "temperature": 0.5}
    model, other_options, history, user_message, response_model = ai.prepare_request(
        prompt, llm_options=llm_options, response_model=UserDetail
    )
    assert model == "test-model"
    assert "temperature" in other_options
    assert "functions" in other_options
    assert "function_call" in other_options
    assert history == [
        {"role": "user", "content": prompt},
    ]
    assert user_message.role == "user"
